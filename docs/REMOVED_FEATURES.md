...
## 3. Tab-key face auth / Enter-key password separation (login screen)

Status: Researched, not implemented.

Investigated whether dank-greeter (the greetd login screen for DMS) exposes
an external hook for triggering a secondary auth factor via keybinding.

Finding: DMS's lock screen has a real PAM auth-factor architecture (proven
by its U2F/fingerprint support, added in DMS 1.6) but no generic third-party
plugin API. dank-greeter (separate project since DMS 1.6) has no documented
external hook for this at all.

Decision: Not pursuing implementation. The underlying problem (fast password
entry racing against camera timeout) is already solved via the fast-fail
timeout separation (8s PAM timeout, 3s no-face detection). This feature
would only add UX convenience, not fix a functional gap, and the maintenance
cost of forking dank-greeter is not justified for that benefit.

Revisit if: dank-greeter or DMS publish a documented external auth-factor
plugin API.